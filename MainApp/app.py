...
@app.route('/') # jadi dikasih int itu buat ngetik di url nya contoh localhost:8000/pembelian/1
@app.route('/<int:id>')
def all_get_methods(id=None):
    # Get Layanan Ulasan
    ulasan_get = get_ulasan()

    # Get Layanan Gambar Ulasan jika image_id disediakan
    if id is not None:
        gambar_ulasan = get_ulasan_gambar(id)
    else:
        gambar_ulasan = None

    # Get Layanan Pembelian
    pembelian_get = get_pembelian()

    # Get Layanan Gambar Pembelian
    gambar_pembelian = get_gambar()

    return render_template('main.html', ulasan=ulasan_get, beli=pembelian_get, gambar_pembelian=gambar_pembelian, gambar_ulasan=gambar_ulasan)
...
@app.route('/delete-pembelian/<string:id>', methods=['DELETE'])
def delete_beli(id):
    # URL endpoint untuk menghapus ulasan berdasarkan ID
    url = f'http://127.0.0.1:5002/api/bookings/{id}'
